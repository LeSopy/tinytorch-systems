...
def test_conv2d_has_backward_function():
    ...
    assert (
        type(output._grad_fn).__name__ == "Conv2dBackward"
...
def test_conv2d_bias_gradient_flow():
    ...
    assert not np.allclose(
        conv.bias.grad.data, 0
    ), "Conv2d bias gradient should be non-zero"
...
def test_maxpool2d_has_backward_function():
    ...
    assert (
        type(output._grad_fn).__name__ == "MaxPool2dBackward"
...
def test_maxpool2d_gradient_flow():
    ...
    assert not np.allclose(
        x.grad.data, 0
    ), "MaxPool2d input gradient should be non-zero"
    ...
    assert (
        num_nonzero < total
    ), "Some gradients should be zero (only max positions get gradients)"
...
def test_conv2d_maxpool2d_chain():
    ...
    assert not np.allclose(
        conv.weight.grad.data, 0
    ), "Conv weight gradient should be non-zero"
    assert not np.allclose(
        conv.bias.grad.data, 0
    ), "Conv bias gradient should be non-zero"
        ...
        for j in range(conv.weight.data.shape[1]):
            for k in range(conv.weight.data.shape[2]):
                for l in range(conv.weight.data.shape[3]):
                    ...
                    # Numerical gradient
                    numerical_grad[i, j, k, l] = (loss_plus - loss_minus) / (
                        2 * epsilon
                    )

    # Compare (relaxed tolerance for explicit loop implementation)
    analytical_grad = conv.weight.grad.data
    relative_error = np.abs(numerical_grad - analytical_grad).max() / (
        np.abs(numerical_grad).max() + 1e-8
    )
